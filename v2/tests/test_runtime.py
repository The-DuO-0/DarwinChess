...
def test_compute_budget_counts_active_runtime_not_paused_sleep():
    ...
    now.advance(60)
    clock.pause()
    now.advance(8 * 60 * 60)  # laptop asleep / run paused
...
def test_league_runs_three_games_but_drains_only_started_colour_pairs():
    ...
    assert not league.all_pairings_complete  # C was never admitted, by design.
...
def test_budget_expiry_enters_safe_colour_pair_drain():
    ...
    assert league.poll_startable() == []  # no B games after the time budget is gone
...
def test_ui_snapshot_exposes_flow_game_turn_and_runtime():
    now = FakeTime()
    ...
    assert payload["flow"][2]["state"] == "current"
