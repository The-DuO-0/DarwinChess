...
def _partial_verdict(*, games: int, score: float, time_ratio: float) -> tuple[str, str]:
    ...
    if games < 12:
        if score >= 0.48 and time_ratio <= 1.35:
            return "PROMISING_SMOKE", "strength is non-regressing so far and compute is within the 1.35x target"
        if time_ratio > 1.45:
            return "SMOKE_COST_WATCH", "below the hard ceiling but still too expensive for a comfortable adoption"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Paired frozen-weight A/B smoke/gate for search-r1 versus search-r2c: "
            "confidence-gated opening +1 with selective root verification. No DB writes."
        )
    )
    ...
    if snapshot_champion != int(args.generation):
        raise RuntimeError(
            f"search-r2c A/B refused: snapshot manifest champion is Gen{snapshot_champion}, "
    ...
    with DarwinRuntime(mode=args.mode, apply_nice=False) as rt:
        ...
        if live_copy_champion != int(args.generation):
            raise RuntimeError(
                f"search-r2c A/B refused: copied DB champion is Gen{live_copy_champion}, "
    ...
    # Before whole games, confirm that the cheaper selective verification still
    # fixes the exact pathology that motivated the revision (Gen54's h4 root).
    probe_b_meter = SearchMeter()
    probe_c_meter = SearchMeter()
    probe_board = chess.Board()
    probe_baseline = make_searcher(False, probe_b_meter).search(probe_board.copy(stack=False), depth=args.depth, top_n=5)
    probe_candidate = make_searcher(True, probe_c_meter).search(probe_board.copy(stack=False), depth=args.depth, top_n=5)
    root_probe = {
        "baseline_move": probe_baseline.move.uci() if probe_baseline.move is not None else None,
        "candidate_move": probe_candidate.move.uci() if probe_candidate.move is not None else None,
        "candidate_stabilized": bool(getattr(probe_candidate, "opening_stabilized", False)),
        "candidate_verified_moves": int(getattr(probe_candidate, "opening_verified_moves", 0) or 0),
        "candidate_extra_nodes": int(getattr(probe_candidate, "opening_extra_nodes", 0) or 0),
    }
    ...
    print(
        f"[search-r2c-ab] Frozen Gen{args.generation}; search-r1 depth={args.depth} vs "
        f"search-r2c selective-root opening +1; pairs={args.pairs}; book moves OFF"
    )
    print(
        "  root probe: "
        f"r1={root_probe['baseline_move']} -> r2c={root_probe['candidate_move']} "
        f"stabilized={root_probe['candidate_stabilized']} "
        f"verified_moves={root_probe['candidate_verified_moves']}"
    )
    ...
    out = report_dir / f"search_r2c_ab_gen{int(args.generation)}_{int(args.pairs)}pairs.json"
    out.write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")

    print("\nSEARCH-R2C FROZEN GEN A/B")
    print("=" * 40)
    print(f"root probe r1 -> r2c: {root_probe['baseline_move']} -> {root_probe['candidate_move']}")
    print(f"root probe verified moves: {root_probe['candidate_verified_moves']}")
