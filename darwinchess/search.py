...
class AlphaBetaSearcher:
    ...
    def _qsearch(self, board: chess.Board, alpha: float, beta: float, depth: int, ply: int) -> float:
        ...
        if not in_check:
            ...
            # Check evasions are not optional, but cap pathological check chains.
            return self.evaluator.evaluate(board)

        # In check, search every legal evasion. Otherwise only tactical moves.
    ...
    def _negamax(self, board: chess.Board, depth: int, alpha: float, beta: float, ply: int) -> tuple[float, list[chess.Move]]:
        ...
        if len(self.tt) >= self.tt_limit:
            # Cheap generational eviction. Search tables are ephemeral by design.
            self.tt.clear()
    ...
    def _root(self, board: chess.Board, depth: int, top_n: int) -> tuple[Candidate | None, list[Candidate]]:
        ...
        for move in self._ordered_moves(board, tt_move=tt_move, policy=policy):
            self._check_time()
            board.push(move)
            # Root moves get a full window so top-k scores used for exploration
            # and policy targets are comparable rather than fail-soft bounds.
